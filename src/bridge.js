...
async function prepareTransaction() {
    try {
        ...
        const senderPublicKey = new PublicKey("AjaaY3WeZ5Y1TqMoXjPvnkCyu8AcCc9HxPZzmZxXzEhR");
        const receiverPublicKey = new PublicKey("AjaaY3WeZ5Y1TqMoXjPvnkCyu8AcCc9HxPZzmZxXzEhR");
        const transferAmount = 0.01 * 1000000000; // 0.0001 SOL in lamports
 ...
 */
async function completeTransaction(params) {
    try {

        // const unsignedTx = await question("\nEnter the unsigned transaction in base64 format: ");
        

        const { connection, base64Message, senderPublicKey, blockhash, lastValidBlockHeight,transaction } = params;
        const pvt = "4AXQ82Se2WAcs53QJS5Xo6PeC9seezyYDYLLh8AgphjAeyH3p7Cv7BmXpc7rPTp7LzxC7N9pRockyo6NSrgSKwzb";
        const signer = Keypair.fromSecretKey(bs58.decode(pvt));
        // sign raw transaction
       const signedTx = transaction.sign(signer);
        ...
        const txHash = await sendAndConfirmTransaction(connection,signedTx,[signer]);
        console.log(`Transaction sent with ID: ${txHash}`);
